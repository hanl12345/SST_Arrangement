...
foreach my $trait (sort keys %prefix_Chr_hash) {
	...
	#Independent significant SNPs (r² < 0.6) 
        my $outpref_1  = $outdir."_Clump/${trait}.chr${chr}IndependentSNVs";
	#AFR_WGS
	...
	system(
		...
		#Lead SNPs (r² < 0.1)
		#my $outpref_2  = $outdir."_Clump/${trait}.chr${chr}LeadSNVs";
		#system(
		#  "bash", "-c",
		# "./plink2 ".
		# "--pfile $genotype_data ".
...
#---------------------------------------------------------------
# Function: collectTraitWithonly1SignificantSNVs
#
# Description:
#   Read a GWAS summary lambda file and identify traits that
#   contain only one significant SNV.
#
# Input:
#   $summaryLambdaFile : Path to the summary lambda file
#                        (e.g., MEGA_AFR_summary_lambda.tsv)
#
# File Format (example columns):
#   column[1] : trait name / phecode
#   column[3] : number of significant SNVs
#
# Output:
#   Returns a reference to a hash where:
#       $Trait_1snvid{trait} = 1
#
# Notes:
#   - The first line of the file is assumed to be a header.
#---------------------------------------------------------------
#---------------------------------------------------------------
sub collectTraitWithonly1SignificantSNVs {
    my ($summaryLambdaFile) = @_;
    ...
    open(my $fh, "<", $summaryLambdaFile)
        or die "Cannot open $summaryLambdaFile: $!";
...
for my $trait (keys %Trait_Count1_chrList_RawGWAS) {
   ...
   print "$trait\t$iSumSNVList\t$iSumSNVList\t$iSumSNVList\n";
   print $out "$trait\t$iSumSNVList\t$iSumSNVList\t$iSumSNVList\n";
}



print $out "Sum\t$SumSNVsCount[0]\t$SumSNVsCount[1]\t$SumSNVsCount[2]\n";
close $out;
  ...
  for(my $jj=0;$jj<@SignifcantSNVsFiles;$jj++)
  {
     ...
     if(defined($Trait_1snvid{$Trait}))
     {
     ...
     else
      {
        ...
        for my $chr (sort keys %{$Trait_Type_Chr_snvid{$Trait}{$clumpTypes[1]}})
        {
	  my @ChrBasedSNVIDList=split(/\t/,$Trait_Type_Chr_snvid{$Trait}{$clumpTypes[1]}{$chr});     
...
my $outPath="AGD35K_SignificantSNVsCountSummary.txt";
open(my $out, ">", "$outPath") or die "Cannot open $outPath: $!";
my @Populations=("AFR","EUR");
print $out "Phenotype\tiTotalSNVs_AFR\tiIndepenfentSNVs_AFR\tiLeadSNVs_AFR";
print $out "iTotalSNVs_EUR\tiIndepenfentSNVs_EUR\tiLeadSNVs_EUR\n";
...
{
   my ($LabValueBasedGWASHitsFile)=@_;
   my %TraitChr_Pos_KnownHits=();
   my $iLineNumber=0;
   open(my $fh, "<", $LabValueBasedGWASHitsFile) or die "Cannot open 
      $LabValueBasedGWASHitsFile: $!";
   while (<$fh>) {
      chomp;
      if($iLineNumber ==0)
...
my $FilePostFix="ClumpedSNVs.txt";
   ...
   for(my $jj=0;$jj<@LeadSNVsFiles;$jj++)
   {
    ...
    my $LabValueBasedGWASHitsFile="./KnownGWASHits_OTG_GWASCatabyMatchtraitFromSource_0204_2026.tsv";
    #my $LabValueBasedGWASHitsFile="./KnownGWASHits_OTG_GWASCatabyMatchDiseaseIDs_0204_2026.tsv"; 
my %TraitChr_Pos_KnownHits=ReadKnownGWASHits_ByTraitFromSourceFile($LabValueBasedGWASHitsFile);
