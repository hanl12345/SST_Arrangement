...
echo -e "idx\tSuperPop\tphenoColList_File\toutBucket_prefix" > "${out_tsv}"
...
for pop in AFR EUR; do
  for i in $(seq -w 0 18); do
    ...
    out_bucket="gs://fc-secure-540f27be-97ea-4ffd-adb7-c195458eb278/Regenie_GWAS35KResultV2/RegenieWorkflowV1/Phecode_${pop}Prob05/Phecode_${pop}"
    echo -e "${idx}\t${pop}\t${pheno_file}\t${out_bucket}" >> "${out_tsv}"
    idx=$((idx + 1))
  done
done
