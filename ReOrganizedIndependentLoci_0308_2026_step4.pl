...
for(my $iT=0;$iT<@PhenotypeList;$iT++)
{
  ...
  my $fullPath="$ClumpSNVDir/AFR_allmergeged_clumps.txt";
      my ($a0a1_AFRRef,
          $SNVIDList_AFRRef,
          $betaFreq_AFRRef,
          $posid_Log10PRef,
          $SNVIDListbypos_AFRRef)=
         ReadGWASclumpedSNVListFileV1($fullPath,
...
my $iEffectiveTraitChr_PosList_AFR=0;
foreach my $trait (sort keys %TraitChr_AlleleListFull_AFR) {
   foreach my $chrpos (sort keys %{$TraitChr_AlleleListFull_AFR{$trait}}) {
	...
	$iEffectiveTraitChr_PosList_AFR++;
     }
}
print "iTotalTraitChr_PosList_AFR:$iTotalTraitChr_PosList_AFR\n";
print "iEffectiveTraitChr_PosList_AFR:$iEffectiveTraitChr_PosList_AFR\n";

#AGD35K_AFR_LabValue_GWAS_ClumpSNVs/EUR_allmergeged_clumps.txt,
#AGD35K_AFR_Phecode_GWAS_ClumpSNVs/EUR_allmergeged_clumps.txt
my %trait_snvid_eur=();
my %trait_refalt_eur=();
my %trait_betafreq_eur=();
for(my $iType=0;$iType<@PhenotypeList;$iType++)
{
  my $EURIndLociFile="AGD35K_AFR_$PhenotypeList[$iType]_GWAS_ClumpSNVs/EUR_allmergeged_clumps.txt";
  my ($trait_snvid_eurRef, 
      $trait_refalt_eurRef,
      $trait_betafreq_eurRef) =
      ReadTraitBasedSignificantVariantInfo(
      $EURIndLociFile,
      \%trait_snvid_eur,
      \%trait_refalt_eur,
     \%trait_betafreq_eur);
     %trait_snvid_eur=%{$trait_snvid_eurRef};
    %trait_refalt_eur=%{$trait_refalt_eurRef};
    %trait_betafreq_eur=%{$trait_betafreq_eurRef};
}


my $GroupHits_Type="DiseaseIDs";
my $OTGGWASHitsFile="AGD35K_Phecode122_0308_2026/Known_LabValue".
        "_GWASHits_OTG_GWASCataby$GroupHits_Type"."_0204_2026.tsv.gz";
...
$OTGGWASHitsFile="AGD35K_Phecode122_0308_2026/Known_PheCode".
        "_GWASHits_OTG_GWASCataby$GroupHits_Type"."_0308_2026.tsv.gz";
%TraitChr_Pos_KnownHits=ReadKnownGWASHits_ByTraitFromSourceFile($OTGGWASHitsFile,\%TraitChr_Pos_KnownHits);
...
my $GWASCatalogHitsFile="AGD35K_Phecode122_0308_2026/Known_LabValue".
        "_GWASHits_GWASCataby$GroupHits_Type"."_0206_2026.tsv.gz";
...
$GWASCatalogHitsFile="AGD35K_Phecode122_0308_2026/Known_PheCode".
        "_GWASHits_GWASCataby$GroupHits_Type"."_0308_2026.tsv.gz";
%TraitChr_Pos_KnownHits_GWASCata=ReadKnownGWASHits_ByTraitFromSourceFile(
           $GWASCatalogHitsFile,
	   \%TraitChr_Pos_KnownHits_GWASCata);
    my ($TraitChr_PosRef,
   ...
   #KnownGWAS_AGD35K_Phecode_EFO_OTGV25.12.tsv
my $LabvalueEFOFile= "AGD35K_Phecode122_0308_2026/AGD35K_OTG_GWASCata_Disease_Labvalue.tsv.gz";
...
my $PhecodeEFOFile= "AGD35K_Phecode122_0308_2026/AGD35K_OTG_GWASCata_Disease_Phecode_0308_2026.tsv.gz";
...
my $JohnLabTraitNameFile="AGD35K_Phecode122_0308_2026/Johns_AGD35K_LabTraits.txt.gz";
my $iLineNum=0;
my %Lab_FullName=();
open(my $fh, "-|", "zcat", $JohnLabTraitNameFile)
     or die "Cannot open $JohnLabTraitNameFile: $!";
while (my $line = <$fh>) {
    if($iLineNum ==0)
    {
      $iLineNum++;
...
my $Phecode1_2File="AGD35K_Phecode122_0308_2026/phecode1.2.tsv.gz";
$iLineNum=0;
my %Code_Label=();
open(my $fh, "-|", "zcat", $Phecode1_2File)
     or die "Cannot open $Phecode1_2File: $!";
while (my $line = <$fh>) {
    if($iLineNum ==0)
    {
      $iLineNum++;
...
my $outsst_AGD35KAFRfile = "sst_AllIndependentloici_AGD35kAFR_Merged_0308_2026.txt";
...
#Estimate correlation of effect between AFR and EUR
#R=0.747, = 8.2637, df = 54, p-value = 3.711e-11
#N=56, QAQC workspace: Han_TriosBasedConcordanceCheck_Metrics_Summary_AGD250K.ipynb 
#Collect the sample size and different phecode
